...
\begin{abstract}
  ...
  frame learning and planning as a form of reasoning.  This paper
  ...
  % Its seminal
  % book~\cite{Goertzel09PLN}, although quite rich and extensive at
  % treating temporal and procedural reasoning

  % has been found to be somewhat lacking formalization.  Some formal

  % suffers from a few drawbacks

  % extensively
  % even though continaing copious information

  % Use PLN


  % This paper offers alternate and complementary definitions of
  % temporal constructs, alongside their inference rules, to carry
...
and planning~\cite{Geisweiller2023ROCCA}.
...
% , grouped into two, those inferring knowledge
% from direct evidence, and those inferring knowledge indirect evince by
% using existing relationships.  At least two types of rules are present
...
knowledge by combining existing abstractions.  In total there are
dozens of inference rules.  For now, we only recall two,
\emph{Implication Direct Introduction} and \emph{Deduction}.

\subsubsection{The Implication Direct Introduction Rule (IDI)} takes
evaluations
% $\TEval$
% links
as premises and produces an implication as conclusion.  It can be
understood as an inductive reasoning rule.  It is formally depicted by
...
premises and produces a third one.  It can be understood as a
deductive reasoning rule.  Depending on the assumptions made there
exists different variations of that rule.  The simplest one is based
on the Markov property % assuming that the probability of $R$
...
$\TSeqOr$ redefined further below. %  In that section we provide a
...
Additionally, we assume that $\lseqand{T}$ and $\lseqor{T}$ are
right-associative.  The $\TLag$ (resp. $\TLead$) operator is
symbolized by an overlined arrow going to the right (resp. to the
left) because it brings the past (resp. the future) into the present.
...
premise, the truth values may optionally be stripped out the rule.
{\small
\begin{prooftree}
...
indirect evidence.
% can be performed by combining the predictive
% implication direct introduction rule with the temporal deduction rule.
% In this section we give an example of temporal reasoning combining
% inferences from direct and indirect evidence.  the predictive
% implication direct introduction rule, to infer a predictive
% implication from direct evidence, and the temporal deduction rule
...
% A
% first predicate implication $P\lpreimp{1} Q$ is inferred using the
% predictive implication direct introduction rule, then combined with a
% second predicate implication $Q\lpreimp{2}R$ already there, to form a
% third one $Q\lpreimp{3}R$ temporal deduction , along with their
% evaluations on two data points, $t_1=1$ and $t_2=2$. The following
% proof tree shows how we apply the PIDI rule
% {\tiny
% \begin{prooftree}
...
% ⊢
% (P⩘ᵀ¹A)↝ᵀ²R
The whole inference tree is given below (using $k=100$ as \emph{Lookahead}
in the truth value formulea).
% See example.scm to get the numerical values
{\tiny
\begin{prooftree}
  \AxiomC{$\!\!\!\!\! P(1) \measeq <\!1, 1\!> \hspace{0.1cm}
    P(2) \measeq <\!1, 1\!> \hspace{0.1cm}
    Q(1\!+\!1) \measeq <\!0, 1\!> \hspace{0.1cm}
    Q(2\!+\!1) \measeq <\!1, 1\!>\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!$}
  \RightLabel{(PIDI)}
  \UnaryInfC{$\!\!\!\!\!\! P \lpreimp{1} Q \measeq <\!0.5, 0.02\!>$}
  \AxiomC{$\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!Q\lpreimp{2} R \measeq <\!0.3, 0.1\!> \hspace{0.1cm}
    P \measeq <\!1, 0.02\!> \hspace{0.1cm}
    Q \measeq <\!0.5, 0.02\!> \hspace{0.1cm}
    R \measeq <\!0.2, 0.5\!>\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!$}
  \RightLabel{(TD)}
  \BinaryInfC{$P \lpreimp{3} R \measeq <\!0.2, 0.018\!>$}
\end{prooftree}}
...
tree like we did for the temporal deduction rule.  The final inference
rule we are trying to build should look like {\small
  \begin{prooftree}
...
After retaining the premises and the conclusion only, and adding back
the truth values, we obtain the following procedural deduction rule:
{\scriptsize
  \begin{prooftree}
    \AxiomC{$\!\!\!\!C_1\!\land\! A_1 \lpreimp{T_1} C_2 \measeq \TTV^{\ \!12}\!\!\!\!\!\!\!\!$}
    \AxiomC{$\!\!\!\!\!\!\!C_2\!\land\!A_2 \lpreimp{T_2} C_3 \measeq \TTV^{\ \!23}\!\!\!\!\!\!\!\!$}
...
% We have introduced an alternate definition of temporal constructs for
% carrying temporal and procedural reasoning with PLN.
Future directions may also include adding inference rules to support
behavior trees; introducing Event Calculus operators as predicate
transformers (similar to how $\TLag$ and $\TLead$ are defined); as
well as supporting temporal intervals and continuous time.
