...
possible to discover heuristics to guide reasoning by a process of
...
modifying a backward chainer by inserting inside its code conditionals
in charge of pruning the search space, with the corresponding
conditions themselves calling the backward chainer to decide whether
to prune a particular search path.  The experiment is further
...
that particular problem only, essentially merely taking advantage of
the January shortcut.
%% TODO: show rule.
...
the performances with inference control are dramatically improved,
ranging from no speed-up to many-fold speed-ups depending on the
difficulty of the query.  More information about the experiment
alongside its code can be found in~\cite{Geisweiller2024ICM}.
