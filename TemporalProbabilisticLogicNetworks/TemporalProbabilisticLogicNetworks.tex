...
\item
  ...
  where $Q$ is conditioned by $P$.  Also, in such implication, $P$ may
  be referred as the \emph{implicant} and $Q$ may be referred as the
  \emph{implicand}.
...
\emph{Fluents}, as predicates equipped with a temporal dimension
...
being an ordered semigroup or such.  In practice so far, we simply
have used integers, thus capturing a discrete notion of time.  Not all
...
future into the present.  As such, the following equivalence holds
...
second predicate has been brought into the present, defined as
...
$\TTrue$ at $t$ requires that $Q$ is $\TTrue$ at time $t+T$.\\
...
definition while remaining perfectly valid.
{\small
\begin{prooftree}
...
implication, as depicted here without truth values by the following
proof tree
{\small
\begin{prooftree}
...
all, and instead show one more variation of that rule over conditional
predicates {\small
\begin{prooftree}
...
\emph{Cognitive Schematics}~\cite{TODO}, of the form
...
directly work with action activation predicates and refer them as
...
\end{prooftree}}
\noindent Note the use of a new rule labeled (I) at the left of the
proof tree.  This rule eliminates independent predicates from an
implication, without modifying the truth value of its conclusion.  Its
use is justified by the fact that $A_2$ is executed
right after reaching $C_2$, thus cannot have an effect on it.\\
...
values, we obtain the following procedural deduction rule {\scriptsize
  ...
  \end{prooftree}}
\noindent with a formula identical to that of the deduction rule.  The
premises filling the dots are therefore
...
decomposed into sub-inferences like it was done with the (I) rule.
...
it introduces no additional assumption, and, if anything else, their
truth values can always be calculated using inference rules based on
direct evidence.
