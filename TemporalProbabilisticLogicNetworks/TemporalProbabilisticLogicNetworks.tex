...
or incorrect.  That paper has been written to remedy that.
...
In that paper, the capital letters $A$, $B$, $C$ represent atoms of
...
a simple truth value corresponds to a beta distribution~\cite{TODO}
with parameters $\alpha(s, c)=\alpha_0 + \frac{s.c.k}{1-c}$ and
...
% The constructs of interest covered in that paper are
% \begin{enumerate}
...
Below is a table of the constructs used in that paper with their
...
\begin{array}{|c|c|c|}
  ...
  A \TBTV & A \measeq \TTV & 10 \\
  \hline
\end{array}
$$
\renewcommand{\arraystretch}{1}

For representing n-ary predicates one may simply use
$P(A_1, \dots, A_n)$ which may be understood as unary predicates
applied on tuples.  Let us now explain their semantics and how to
interpret their truth values.
...
\item
  ...
  knowing $P$, referring to their satisfying sets to be precise.
...
\item $P \measeq \TTV$ states that the prevalence of $P$ being
  $\TTrue$ has the second order probability described by $\TTV$.
  % It is
  % equivalent to $P \limp \mathbb{True} \measeq \TTV$ where
...
from direct evidence, while rules from the latter infer knowledge by
combining existing abstractions.  There are dozens of inference rules,
for now we only recall two, \emph{Implication Direct Introduction} and
\emph{Deduction}.
...
% $\TEval$
% links
as premises and produces an implication as conclusion, formally
...
\end{prooftree}}
\noindent Assuming perfectly reliable direct
evidence\footnote{Perfectly reliable evidence have a confidence of 1.
  Dealing with unreliable evidence involves using convolution products
  and is outside of the scope of this paper.}  then the resulting
simple truth value can be calculated as follows
...
%% $$\prob(R|Q,P) = \prob(R|Q)$$
%% $$\prob(R|\neg Q,P) = \prob(R|\neg Q)$$
\noindent The reader may notice that three additional premises have been added,
corresponding to the probabilities $\prob(P)$, $\prob(Q)$ and
$\prob(R)$.  This is a consequence of the Markov property.  The exact
formula for that variation will not be recalled here but it merely
derives from
$$\prob(R|P) = \prob(R|Q,P)\times\prob(Q|P) + \prob(R|\neg
Q,P)\times\prob(\neg Q|P)$$
More information can be found in Chapter X of~\cite{TODO}.
