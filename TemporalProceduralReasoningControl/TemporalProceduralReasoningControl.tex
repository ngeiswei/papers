...
\begin{array}{|c|c|c|}
  ...
  \hline
  \TEval(P, \TList(X_1, ..., X_n)) & P(X_1, ..., X_n) & 1 \\
...
build inference rules for procedural reasoning

{\tiny
  \begin{prooftree}
    \AxiomC{$P \MAnd \MDo{A} \MPredImpl^{T_1} Q$}
    \RightLabel{(PI)}
    \UnaryInfC{$P \MAnd \MDo{A} \MImpl
      \overleftarrow{Q}^{T_1}$}
    \AxiomC{$\MDo{B}$}
    \RightLabel{(S)}
    \UnaryInfC{$\overleftarrow{\MDo{B}}^{T_1}$}
    \RightLabel{(C)}
    \BinaryInfC{$P \MAnd \MDo{A} \MAnd
      \overleftarrow{\MDo{B}}^{T_1} \MImpl
      \overleftarrow{Q}^{T_1} \MAnd \overleftarrow{\MDo{B}}^{T_1}$}
    \AxiomC{$Q \MAnd \MDo{B} \MPredImpl^{T_2} R$}
    \RightLabel{(PI)}
    \UnaryInfC{$Q \MAnd \MDo{B} \MImpl \overleftarrow{R}^{T_2}$}
    \RightLabel{(S)}
    \UnaryInfC{$\overleftarrow{Q}^{T_1} \MAnd
      \overleftarrow{\MDo{B}}^{T_1} \MImpl
      \overleftarrow{R}^{T_1+T_2}$}
    %% \AxiomC{$P \MAnd \MDo{A}$}
    %% \AxiomC{$\overleftarrow{\MDo{B}}^{T_1}$}
    %% \RightLabel{(C)}
    \AxiomC{$P \MAnd \MDo{A} \MAnd \overleftarrow{\MDo{B}}^{T_1}$}
    \AxiomC{$Q \MAnd \MDo{B}$}
    \RightLabel{(S)}
    \UnaryInfC{$\overleftarrow{Q}^{T_1} \MAnd \overleftarrow{\MDo{B}}^{T_1}$}
    \AxiomC{$R$}
    \RightLabel{(S)}
    \UnaryInfC{$\overleftarrow{R}^{T_1+T_2}$}
    \RightLabel{(D)}
    \QuinaryInfC{$P \MAnd \MDo{A} \MAnd \overleftarrow{\MDo{B}}^{T_1} \MImpl \overleftarrow{R}^{T_1+T_2}$}
    \RightLabel{(IP)}
    \UnaryInfC{$P \MAnd \MDo{A} \MSeqAnd^{T_1}
      \MDo{B} \MPredImpl^{T_1+T_2} R$}
\end{prooftree}}
...
specifically meant that they are as XXX and cover as many situations
as possible.
