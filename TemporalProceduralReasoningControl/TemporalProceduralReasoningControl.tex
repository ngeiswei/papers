...
\emph{Fluents}, as regular predicates with a temporal dimension
...
$\TLead$, is similar to the Yesterday $Y$, respectively Tomorrow $X$,
operator of Linear Temporal Logic \cite{TODO}, with the difference
that it applies over an expression of predicate type as opposed to an
expression of boolean type.  It also takes a temporal difference as
extra argument, which, in the case of a discrete time, would amount to
calling the $Y$ or $X$ operators multiple times.
